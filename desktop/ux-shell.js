...
(() => {
  ...
  const groups = [
    {
      ...
      icon: '⌂',
      open: true,
      items: ['inicio', 'assistantflow', 'guidedproject']
    },
    {
      title: '1. Preparar estudio',
      icon: '01',
      open: true,
      items: ['profiles', 'dataimport', 'legacyimport', 'templates', 'samplesize', 'diagnostics', 'multidiag', 'missingpro']
    },
    {
      title: '2. Validar instrumento',
      icon: '02',
      open: true,
      items: ['aiken', 'reliability', 'efa']
    },
    {
      title: '3. Confirmar estructura',
      icon: '03',
      open: true,
      items: ['cfa', 'latencia', 'motorpro', 'advanced', 'estimadorguide', 'syntaxpro', 'semmontecarlo', 'modelcheck', 'modelcompare']
    },
    {
      title: '4. Interpretar y reportar',
      icon: '04',
      open: true,
      items: ['qualitydashboard', 'resultcenter', 'conclusionassistant', 'articletables', 'reportapa', 'finalcheck', 'journalready']
    },
    {
      title: '5. Gestionar proyecto',
      icon: '05',
      open: false,
      items: ['projects', 'migration', 'history', 'versions', 'sync', 'teams', 'peerreview', 'activitylog', 'tasks', 'kanban', 'approvals', 'conflicts', 'collabexport', 'notifications']
    },
    {
      title: 'Ayuda y configuración',
      icon: '?',
      open: false,
      items: ['helpcenter', 'methodguide', 'guia', 'referencias', 'accessibility', 'privacy', 'encryption', 'appsettings', 'systemcheck', 'advancedsettings', 'updates', 'acerca']
    },
    {
      title: 'Administración técnica',
      icon: '⚙',
      open: false,
      items: ['auth', 'institutionlib', 'backendtests', 'adminpanel', 'oidc', 'telemetry', 'releases', 'auditpackage', 'servermonitor', 'backuprestore', 'serverwizard', 'incidents', 'userdocs', 'e2e', 'installer', 'scheduledbackup', 'loadtesting', 'a11yaudit', 'betametrics', 'releasecandidate', 'regression', 'securityreview', 'betaready']
    }
  ];
  ...
  brand.innerHTML = '<div class="sidebar-brand-mark">VS</div><div><strong>ValiStruct</strong><span>Ruta de trabajo</span></div>';
  ...
  footer.innerHTML = '<strong>Beta Desktop</strong><span>Flujo guiado de validación</span>';
  ...
  workflow.innerHTML = `
    <div class="workflow-intro">
      <div>
        <span class="eyebrow">RUTA RECOMENDADA</span>
        <h2>¿Qué quieres hacer hoy?</h2>
        <p>Avanza por el proceso de validación sin perderte entre herramientas. Puedes entrar directamente a cualquier etapa.</p>
      </div>
      <button type="button" class="workflow-primary" data-target="guidedproject">Iniciar proyecto guiado</button>
    </div>
    <div class="workflow-steps">
      <button type="button" class="workflow-card" data-target="dataimport"><span>01</span><strong>Preparar datos</strong><small>Importar, revisar calidad y tamaño muestral</small></button>
      <button type="button" class="workflow-card" data-target="aiken"><span>02</span><strong>Validar contenido</strong><small>V de Aiken y confiabilidad inicial</small></button>
      <button type="button" class="workflow-card" data-target="efa"><span>03</span><strong>Explorar estructura</strong><small>AFE y diagnóstico de dimensiones</small></button>
      <button type="button" class="workflow-card" data-target="cfa"><span>04</span><strong>Confirmar modelo</strong><small>AFC, SEM, ajuste y validez de constructo</small></button>
      <button type="button" class="workflow-card" data-target="reportapa"><span>05</span><strong>Reportar resultados</strong><small>Interpretación, APA 7 y preparación para revista</small></button>
    </div>
    <div class="quick-actions">
      ...
      <button type="button" data-target="helpcenter">Ayuda</button>
    </div>`;
  ...
  if (topbar) topbar.classList.add('desktop-topbar');
